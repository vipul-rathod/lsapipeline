...
class PrimaryPublishHook(Hook):
	...
	def _do_maya_publish(self, task, work_template, comment, thumbnail_path, sg_task, progress_cb):
		...
		## Add publishVersion custom attr tag to OCEAN_hrc if exist and setAttr to published version to cater for FX version checker.
		if cmds.objExists('OCEAN_hrc'):
			publishVer = int( publish_path.split('.')[-2].strip('v') )
			if not cmds.objExists('OCEAN_hrc.publishVersion'):
				cmds.addAttr('OCEAN_hrc', longName = 'publishVersion', attributeType = 'long')
				cmds.setAttr('OCEAN_hrc.publishVersion', keyable = True)

			cmds.setAttr('OCEAN_hrc.publishVersion', publishVer)
		...
		## Bake all oceanLock locators regardless of interactive or not after saving working file
		if cmds.objExists('BOAT_OceanLocators_hrc'):
			oceanLock = cmds.listRelatives('BOAT_OceanLocators_hrc', type = 'locator', allDescendents = True, fullPath = True)
			if oceanLock:
				oceanLock = [cmds.listRelatives(loc, parent = True, fullPath = True)[0] for loc in oceanLock if loc.endswith('_boatOceanLocShape')]
				if oceanLock:
					progress_cb(25.0, "Baking ocean locators...")
					...
					cmds.bakeResults(oceanLock, simulation = True, time = (minFrame, maxFrame), sampleBy = 1, disableImplicitControl = True, preserveOutsideKeys = True, sparseAnimCurveBake = False, removeBakedAttributeFromLayer = False, bakeOnOverrideLayer = False, minimizeRotation = True, controlPoints = False, shape = False, at = ["tx", "ty", "tz", "rx", "ry", "rz"])
		...
		if 'Blck' in publish_path:
			publish_name = '%s_LAYOUT' % publish_name
		else:
