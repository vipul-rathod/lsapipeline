...
class PrimaryPublishHook(Hook):
    ...
    def _do_maya_publish(self, task, work_template, comment, thumbnail_path, sg_task, progress_cb):
        ...
        debug(None, method = 'init_app', message = 'scene_path: %s' % scene_path, verbose = False)
        ...
        debug(None, method = 'init_app', message = 'publish_path: %s' % publish_path, verbose = False)
        ...
        if not 'SRFVar_' in scene_path:
            ...
            debug(None, method = 'init_app', message = 'REGISTERING SURFACE VARIATION...', verbose = False)
            self._register_publish(publish_path, 
                                   '%sSHD_surfVar%s' % (publish_name,  scene_path.split('SRFVar_')[-1].split('\\')[0]), 
                                   sg_task, 
                                   fields["version"], 
                                   output["tank_type"],
                                   comment,
