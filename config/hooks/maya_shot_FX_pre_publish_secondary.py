    ...
    def execute(self, tasks, work_template, progress_cb, **kwargs):
        ...
        for task in tasks:
            ...
            if item["type"] == "fx_caches":
                debug(app = None, method = 'shotFX_PrePublishHook', message = 'fx_caches type found', verbose = False)
                errors.extend(self._validate_item_for_publish(item))
            elif item["type"] == "nparticle_caches":
                debug(app = None, method = 'shotFX_PrePublishHook', message = 'nparticle_caches type found', verbose = False)
                errors.extend(self._validate_item_for_publish(item))
            elif item["type"] == "fx_renders":
                debug(app = None, method = 'shotFX_PrePublishHook', message = 'fx_renders type found', verbose = False)
                errors.extend(self._validate_item_for_publish(item))
            else:
                # don't know how to publish this output types!
                errors.append("Don't know how to publish this item! \nPlease contact your supervisor or lead..." % output["name"])          
