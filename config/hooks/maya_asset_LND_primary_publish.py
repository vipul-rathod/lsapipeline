...
class PrimaryPublishHook(Hook):
    ...
    def _do_maya_publish(self, task, work_template, comment, thumbnail_path, sg_task, progress_cb):
        ...
        ## DO THE CORE ARCHIVE XML PUBLISH NOW
        if cmds.objExists('CORE_ARCHIVES_hrc'):
            xmlPath = '%s/coreArchive/corearchives.xml' % publish_path.split('maya')[0]
            writeXML.writeCoreData(pathToXML = xmlPath)
            cmds.delete('CORE_ARCHIVES_hrc')
