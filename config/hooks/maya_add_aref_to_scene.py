...
class AddFileToScene(tank.Hook):
    ...
    def add_file_to_maya(self, file_path, shotgun_data):
        ...
        texture_extensions = [".png", ".jpg", ".jpeg", ".exr", ".cin", ".dpx",".psd", ".tiff", ".tga"]
        ...
        if ext in [".ma", ".mb"]:
            ...
            if not cmds.objExists(assetName):
                ...
                cmds.setAttr('%s.definition' % assetName,  assemblyDefPath, type = 'string')
            else:
                cmds.warning('Asset %s already exists in scene, try using the sceneBreakdown tool to update instead....' % assetName)
