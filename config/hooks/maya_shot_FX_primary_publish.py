...
class PrimaryPublishHook(Hook):
    ...
    def _do_maya_publish(self, task, work_template, comment, thumbnail_path, sg_task, progress_cb):
        ...
        debug(app = None, method = '_do_maya_publish', message = 'Primary publish in progress...', verbose = False)
        progress_cb(0.0, "Finding scene dependencies", task)
        dependencies = self._maya_find_additional_scene_dependencies()
        ## Get scene path
        scene_path = os.path.abspath(cmds.file(query=True, sn= True))
        debug(app = None, method = '_do_maya_publish', message ='scene_path: %s' % scene_path, verbose = False)
        ...
        if os.path.exists(publish_path):
           ...
           progress_cb(10.0, 'Found existing publish_path: %s' % publish_path)
           progress_cb(15.0, 'Adjusting publish_path now..')
           path = '\\'.join(publish_path.split('\\')[0:-1])
           getfiles = os.listdir(path)
           if 'Keyboard' in getfiles:
               getfiles.remove('Keyboard')
           
           progress_cb(20.0, 'Removing Legacy ma publish files now..')
           ...
           progress_cb(25.0,'NewPublishPath: %s' % publish_path)
        
        ## Save the scene so we have a valid os.rename path
        progress_cb(35.0, "Saving the current working scene to new vers num....")
        publish_name = self._get_publish_name(publish_path, publish_template, fields)
        debug(app = None, method = '_do_maya_publish', message ='publish_name: %s' % publish_name, verbose = False)
        ...
        debug(app = None, method = '_do_maya_publish', message ='REVERTING TO MA: publish_path: %s' % publish_path, verbose = False)
        cmds.file(save=True, force=True, type = 'mayaAscii')
        progress_cb(100)

        scene_path = os.path.abspath(cmds.file(query=True, sn= True))
        debug(app = None, method = '_do_maya_publish', message =  'YOU MAYA... scene_path: %s' % scene_path, verbose = False)
