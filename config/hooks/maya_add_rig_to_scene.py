...
class AddFileToScene(tank.Hook):
    ...
    def add_file_to_maya(self, file_path, shotgun_data):
        ...
        if ext in [".ma", ".mb"]:
            ...
            if 'CHAR_' in namespace:
                if not cmds.objExists('CHARS_hrc'):
                    cmds.group(n = 'CHARS_hrc', em = True)
                cmds.file(file_path, r = True, loadReferenceDepth = "all", options = 'v=0', ns = namespace, f = True)
                for grp in cmds.ls(assemblies=True, long= True):
                    if namespace in grp:
                        cmds.parent(grp, 'CHARS_hrc')
            elif 'PROP_' in namespace:
                if not cmds.objExists('PROPS_hrc'):
                    cmds.group(n = 'PROPS_hrc', em = True)
                cmds.file(file_path, r = True, loadReferenceDepth = "all", options = 'v=0', ns = namespace, f = True)
                for grp in cmds.ls(assemblies=True, long= True):
                    if namespace in grp:
                        cmds.parent(grp, 'PROPS_hrc')
        else:
