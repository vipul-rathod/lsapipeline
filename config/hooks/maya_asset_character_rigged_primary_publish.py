...
class PrimaryPublishHook(Hook):
    ...
    def _do_maya_publish(self, task, work_template, comment, thumbnail_path, sg_task, progress_cb):
        ...
        try:
            ...
            newPublishSceneName = '%s.v%s%s' % (publish_name, padding, fields['version'])
            cmds.file(rename = newPublishSceneName)
            ...
            if cmds.objExists('blendshapes_hrc'):
                ## Put the fn thing to world cause deleting it is causing major drama's on saving the work file again trying to undo !!!!
                #cmds.parent('blendshapes_hrc' , world = True)
                cmds.undoInfo(state = 1)
                cmds.delete('blendshapes_hrc')
                cmds.select([charRoot for charRoot in cmds.ls(type = 'transform') if 'CHAR_' in charRoot], r = True)
                
                ## Make sure the synoptics are being exported....
                ## check for the CHAR object set and add this to the selection for export
                for each in cmds.ls(type = 'objectSet'):
                    if 'CHAR_' in each:
                        cmds.select(each , add = True)
                        
                cmds.file(publish_path, force = True, options = "v=0;", typ = "mayaBinary", es = True)
                cmds.undo()
                ...
                ## The rename moves the file for us...
