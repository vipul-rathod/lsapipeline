...
class AddFileToScene(tank.Hook):
    ...
    def add_file_to_maya(self, file_path, shotgun_data):
        ...
        if ext in [".ma", ".mb"]:
            ...
            # Use the entity name as the namespace when creating the reference:
            sg_link = shotgun_data['entity']
            
            # Ensure the namespace ends in a number so that the entity name is clear in namespaces 
            # of references of the same entity when the entity name also ends in a number (otherwise
            # the entity name will be incremented!)
            namespace = "%s_1" % sg_link['name']
            
            # create the reference using default settings plus the namespace:
            cmds.file(file_path, 
                      reference=True, 
