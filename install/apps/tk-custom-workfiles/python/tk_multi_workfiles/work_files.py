...
#            dir(self._context) ['_Context__additional_entities', '_Context__entity', '_Context__project', '_Context__step', '_Context__task', '_Context__tk', '_Context__user', '__class__', '__deepcopy__', '__delattr__', '__dict__', '__doc__', '__eq__', '__format__', '__getattribute__', '__hash__', '__init__', '__module__', '__ne__', '__new__', '__reduce__', '__reduce_ex__', '__repr__', '__setattr__', '__sizeof__', '__str__', '__subclasshook__', '__weakref__', '_entity_fields_cache', '_fields_from_entity_paths', '_fields_from_shotgun', '_fields_from_template_tree', 'additional_entities', 'as_template_fields', 'create_copy_for_user', 'entity', 'entity_locations', 'filesystem_locations', 'project', 'sgtk', 'shotgun_url', 'step', 'tank', 'task', 'user']
#            dir(self._app) ['_Application__engine', '_Application__instance_name', '_TankBundle__context', '_TankBundle__descriptor', '_TankBundle__environment', '_TankBundle__execute_hook_internal', '_TankBundle__frameworks', '_TankBundle__module_uid', '_TankBundle__post_process_settings_r', '_TankBundle__resolve_hook_path', '_TankBundle__resolve_setting_value', '_TankBundle__settings', '_TankBundle__sg', '_TankBundle__tk', '__class__', '__delattr__', '__dict__', '__doc__', '__format__', '__getattribute__', '__hash__', '__init__', '__module__', '__new__', '__reduce__', '__reduce_ex__', '__repr__', '__setattr__', '__sizeof__', '__str__', '__subclasshook__', '__weakref__', '_can_change_version', '_can_save_as', '_destroy_frameworks', 'cache_location', 'can_save_as', 'context', 'description', 'descriptor', 'destroy_app', 'disk_location', 'display_name', 'documentation_url', 'engine', 'ensure_folder_exists', 'execute_hook', 'execute_hook_by_name', 'execute_hook_expression', 'execute_hook_method', 'frameworks', 'get_setting', 'get_setting_from', 'get_template', 'get_template_by_name', 'get_template_from', 'icon_256', 'import_module', 'init_app', 'instance_name', 'log_debug', 'log_error', 'log_exception', 'log_info', 'log_warning', 'name', 'post_engine_init', 'settings', 'sgtk', 'shotgun', 'show_change_version_dlg', 'show_change_work_area_dlg', 'show_file_manager_dlg', 'show_save_as_dlg', 'support_url', 'tank', 'version']
            ...
            self.assetType = self._app.shotgun.find_one('Asset', [['code', 'is', self.assetName]], fields=['sg_asset_type'])['sg_asset_type']
            self.assetId = self._context._Context__entity['id']
            self.assetTaskId = self._context._Context__task['id']
            

            if self.step == 'Model' and self.assetType == 'Environment':
                assetInfoAttr_Var = assetInfoAttr.AssetInfoAttr()
                lodHighGroup = cmds.group(n='%s_LODhigh_hrc' % self.assetName, em=1)
                assetInfoAttr_Var.assetNameAttr(self.assetName, lodHighGroup)
                assetInfoAttr_Var.assetTaskAttr(self.step, lodHighGroup)
                assetInfoAttr_Var.assetTypeAttr(self.assetType, lodHighGroup)
                assetInfoAttr_Var.assetIdAttr(self.assetId, lodHighGroup)
                assetInfoAttr_Var.assetTaskIdAttr(self.assetTaskId, lodHighGroup)
                assetInfoAttr_Var.assetLODTypeAttr('High', lodHighGroup)
                cmds.select(cl=1)
                
                lodLowGroup = cmds.group(n='%s_LODlow_hrc' % self.assetName, em=1)
                assetInfoAttr_Var.assetNameAttr(self.assetName, lodLowGroup)
                assetInfoAttr_Var.assetTaskAttr(self.step, lodLowGroup)
                assetInfoAttr_Var.assetTypeAttr(self.assetType, lodLowGroup)
                assetInfoAttr_Var.assetIdAttr(self.assetId, lodLowGroup)
                ...
                assetInfoAttr_Var = assetInfoAttr.AssetInfoAttr()
                rigTopGroup = cmds.group(n='%s_%s_hrc' % (self.assetName, self.step), em=1)
                assetInfoAttr_Var.assetNameAttr(self.assetName, rigTopGroup)
                assetInfoAttr_Var.assetTaskAttr(self.step, rigTopGroup)
                assetInfoAttr_Var.assetTypeAttr(self.assetType, rigTopGroup)
                assetInfoAttr_Var.assetIdAttr(self.assetId, rigTopGroup)
                assetInfoAttr_Var.assetTaskIdAttr(self.assetTaskId, rigTopGroup)
                cmds.select(cl=1)
