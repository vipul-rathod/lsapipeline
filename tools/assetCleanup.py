...
"""
CLEANUP:
There is a python script in the tools folder that can be sent to each remote artist for cleanup
software/lsapipeline/tools/assetCleanup.py

USAGE:
Copy script to maya scripts folder
Make sure you have the top group for the asset to clean selected then in a python script editor type

import assetCleanup as cleanup
cleanup.cleanup()
"""
def debug(app= '', method = '', message = '', verbose = False):
    ...
    if not app:
        if verbose:
            print ('Method: %s:\t\t%s' % (method, message))
        else:
            pass
    else:
        try:
...
def shotCleanupLights():
    ...
    for eachLight in getLights:
        try:
            if cmds.listRelatives(eachLight, parent = True)[0] != 'LIGHTS_hrc':
...
def _geoSuffixCheck(items):
    ...
    for eachItem in items:
        debug(None, method = '_geoSuffixCheck', message = 'Checking suffix now...eachItem: %s' % eachItem,  verbose = False)
...
def cleanUp(items=[], instanceCheck = True, checkShapes = True, history = True, pivots = True, freezeXFRM = True, smoothLvl = True, tagSmoothed = True, checkVerts = True, 
            renderflags = True, deleteIntermediate = True, turnOffOpposite = True, shaders = True, hyperLayouts = True, removeNS = False, coreArchives = False, defaultRG = True,
    ...
    ## Global scene cleans
    checkingMessage(checkingWhat = 'SHAPENAMES:%s HISTORY:%s, PIVOTS:%s, \nXFORMS:%s, SMOOTHLVL1:%s, TAGSMOOTHED:%s, \nVERTS:%s, RENDERFLAGS:%s, INTERMEDIATES:%s, \nOPPOSITEATTR:%s, INSTANCES:%s, SHADERS:%s\n HYPERLAYOUTS:%s, REMMOVENS:%s, COREARCHIVES:%s' % (checkShapes, history, pivots, freezeXFRM, smoothLvl, tagSmoothed, checkVerts, renderflags, deleteIntermediate, turnOffOpposite, instanceCheck, shaders,  hyperLayouts, removeNS, coreArchives), start = True,  failed = False)
...
def cleanup():
    """
    Remote tool for operators working off site to check their scene before sending back to server
    """
    mySel = cmds.ls(sl=True)
    if mySel and '_hrc' in mySel[0]:
        myFakeTemplate = [{'type': 'mesh_group', 'name': u'|%s' % mySel[0]}]
        
        ## HARDFAIL DUPLICATE NAMES
        if not duplicateNameCheck():
            cmds.warningr("Duplicate names found please fix before publishing.\nCheck the outliner for the duplicate name set.")
            return -1
        ## HARD FAIL GEO SUFFIX
        else:
            checkSceneGeo = _geoSuffixCheck(myFakeTemplate)
            if not checkSceneGeo:
                cmds.warning("Incorrect Suffixes found! Fix suffixes before publishing.\nCheck the outliner for the duplicate name set.")
            else:
                cleanUp(items=myFakeTemplate, instanceCheck = True, checkShapes = True, history = True, pivots = True, freezeXFRM = True, smoothLvl = True, tagSmoothed = True, checkVerts = True, 
                            renderflags = True, deleteIntermediate = True, turnOffOpposite = True, shaders = True, hyperLayouts = True, removeNS = False, coreArchives = False, defaultRG = True,
                            lightingCleanup = False)
    else:
        cmds.warning('You must have your root _hrc group node selected before running this tool. eg: BBB_The_Marina_BLD_hrc or CHAR_Sydney_hrc')